...
namespace SMT.HRM.BLL
{
    public class EmployeeLeaveRecordBLL : BaseBll<T_HR_EMPLOYEELEAVERECORD>, ILookupEntity, IOperate
    {
            ...
            try
            {
                ...
                if (strCheckState == Convert.ToInt32(SMT.HRM.DAL.CheckStates.Approved).ToString())
                {
                    ...
                    IQueryable<T_HR_ATTENDANCERECORD> entArs = from r in dal.GetObjects<T_HR_ATTENDANCERECORD>()
                                                               where r.EMPLOYEEID == ent.EMPLOYEEID && r.ATTENDANCEDATE >= dtStart && r.ATTENDANCEDATE <= dtEnd
                                                               select r;
                    ...
                    foreach (T_HR_ATTENDANCERECORD item in entArs)
                    {
                        ...
                        try
                        {
                            DealAbnormRecord(ent, bllAttendRecord, bllAbnormRd, item, entAbnormRecords);
                        }
                        catch (Exception ex)
                        {
        ...
        private void DealAbnormRecord(T_HR_EMPLOYEELEAVERECORD ent, AttendanceRecordBLL bllAttendRecord, AbnormRecordBLL bllAbnormRd, T_HR_ATTENDANCERECORD item, IQueryable<T_HR_EMPLOYEEABNORMRECORD> entAbnormRecords)
        {
            ...
            {
                //需根据请假时间判断是否要删除掉考勤异常
                DateTime datLevestart = ent.STARTDATETIME.Value;//长日期格式2013/2/16 8:30:00
                DateTime datLeveEnd = ent.ENDDATETIME.Value;//长日期格式2013/2/16 8:30:00
                ...
                if (q.Count() > 0)
                {
                        ...
                        if (!string.IsNullOrEmpty(defineTime.FIRSTSTARTTIME))
                        {
                            ...
                            if (!string.IsNullOrEmpty(defineTime.FIRSTENDTIME))
                            {
                                ...
                                if (datLevestart <= ShiftstartDateAndTime
                                    && datLeveEnd >= FirstEndDateAndTime)
                                {
                                    //消除第一段异常生成的签卡
                                    DeleteSigFromAbnormal(AbnormRecorditem);
                                    //消除第一段打卡时间异常考勤
                                    bllAbnormRd.Delete(AbnormRecorditem);
                                    //第一段考勤时间标记为请假
                        ...
                        if (!string.IsNullOrEmpty(defineTime.SECONDSTARTTIME))
                        {
                            ...
                            if (!string.IsNullOrEmpty(defineTime.SECONDENDTIME))
                            {
                                ...
                                if (datLevestart <= SecondStartDateAndTime
                                    && datLeveEnd >= SencondEndDateAndTime)
                                {
                                    //消除第二段异常生成的签卡
                                    DeleteSigFromAbnormal(AbnormRecorditem);
                                    //消除第二段打卡时间异常考勤
                    ...
                    if (!string.IsNullOrEmpty(defineTime.NEEDSECONDOFFCARD) && defineTime.NEEDSECONDOFFCARD == "2")//以集团打卡举例，第二段下班打卡17:30
                    {
                        if (!string.IsNullOrEmpty(defineTime.SECONDSTARTTIME))
                        {
                            DateTime dtShiftFirstStart3 = DateTime.Parse(defineTime.SECONDSTARTTIME);
                            DateTime dtShiftstart3 = new DateTime(dtDateAbnorm.Year, dtDateAbnorm.Month, dtDateAbnorm.Day, dtShiftFirstStart3.Hour, dtShiftFirstStart3.Minute, dtShiftFirstStart3.Second);

                            if (!string.IsNullOrEmpty(defineTime.SECONDENDTIME))
                            {
                                DateTime dtShiftEndStart3 = DateTime.Parse(defineTime.SECONDENDTIME);
                                DateTime dtShiftEnd3 = new DateTime(dtDateAbnorm.Year, dtDateAbnorm.Month, dtDateAbnorm.Day, dtShiftEndStart3.Hour, dtShiftEndStart3.Minute, dtShiftEndStart3.Second);

                                if (datLevestart <= dtShiftstart3
                                    && datLeveEnd >= dtShiftEnd3)
                                {
                                    //消除第三段异常生成的签卡
                                    DeleteSigFromAbnormal(AbnormRecorditem);
                                    //消除第三段打卡时间异常考勤
