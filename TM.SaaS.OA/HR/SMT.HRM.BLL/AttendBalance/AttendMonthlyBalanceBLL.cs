...
namespace SMT.HRM.BLL
{
    public partial class AttendMonthlyBalanceBLL : BaseBll<T_HR_ATTENDMONTHLYBALANCE>
    {
            ...
            try
            {
                ...
                while (!TF.EndOfData)
                {
                    try
                    {
                        ...
                        entTemp.NEEDATTENDDAYS = GetNeedAttendDays(dtStart, dtEnd, entCurrEmp.OWNERCOMPANYID,entCurrEmp.EMPLOYEEID, entAttSol);
        ...
        public string CalculateEmployeeAttendanceMonthlyByCompanyID(string strCurDateMonth, string strCompanyID, string BalanceEmployeeid)
        {
            try
            {
                ...
                if (entEmployees.Count() == 0)
                {
                    return "通过公司未获取到员工，请确认";
                }

                return CalculateEmployeeAttendMonthlyBalance(ref dtStart, ref dtEnd, entEmployees, strCompanyID, "1", BalanceEmployeeid);
            }
            catch (Exception ex)
            {
                Utility.SaveLog(ex.ToString());
            }
            return "结算考勤一次，请联系管理员";
        ...
        public string CalculateEmployeeAttendanceMonthlyByDepartmentID(string strCurDateMonth, string strDepartmentID, string BalanceEmployeeid)
        {
            try
            {
                ...
                if (entEmployees.Count() == 0)
                {
                    return "通过部门未获取到员工，请确认";
                }

                return CalculateEmployeeAttendMonthlyBalance(ref dtStart, ref dtEnd, entEmployees, entCompany.COMPANYID, "2", BalanceEmployeeid);
            }
            catch (Exception ex)
            {
                Utility.SaveLog(ex.ToString());
        ...
        public string CalculateEmployeeAttendanceMonthlyByPostID(string strCurDateMonth, string strPostID, string BalanceEmployeeid)
        {
            try
            {
                ...
                return CalculateEmployeeAttendMonthlyBalance(ref dtStart, ref dtEnd, entEmployees, entCompany.COMPANYID, "3", BalanceEmployeeid);
            }
            catch (Exception ex)
            {
                Utility.SaveLog(ex.ToString());
        ...
        /// </summary>
        /// <param name="strCurDateMonth">月份(年-月：2010-7)</param>
        /// <param name="strBalancePostID">需进行考勤结算的员工的员工ID</param>
        public string CalculateEmployeeAttendanceMonthlyByBalacePostID(string strCurDateMonth, string strBalancePostID, string BalanceEmployeeid)
        {
            try
            {
                if (string.IsNullOrEmpty(strBalancePostID))
                {
                    return "请选择结算岗位";
                }
                ...
                dtEnd = dtStart.AddMonths(1).AddDays(-1);
                 ...
                 List<T_HR_EMPLOYEE> entEmployees=getAllEmployeeByBlancePost(strBalancePostID);
                ...
                return CalculateEmployeeAttendMonthlyBalance(ref dtStart, ref dtEnd, entEmployees.AsQueryable(), strCompanyID, "4", BalanceEmployeeid);
            }
            catch (Exception ex)
            {
                Utility.SaveLog(ex.ToString());
        ...
        private List<T_HR_EMPLOYEE> getAllEmployeeByBlancePost(string strBalancePostID)
        {
            List<EpmloyeeBlanceSet> PostAll = new List<EpmloyeeBlanceSet>();
            if (!string.IsNullOrEmpty(strBalancePostID))
            {
                PostAll = (from ent in dal.GetObjects<T_HR_EMPLOYEESALARYPOSTASIGN>()
                           join b in dal.GetObjects<T_HR_BALANCEPOSTDETAIL>()
                           on ent.EMPLOYEESALARYPOSTASIGNID equals b.BALANCEPOSTASIGNID
                           where ent.BALANCEPOSTID == strBalancePostID
                           && ent.CHECKSTATE == "2"
                           && b.ATTENDANCESET == "1"//考勤结算设置
                           && b.EDITSTATE == "1"
                           select new EpmloyeeBlanceSet
                           {
                               employeeid = "",
                               BalancePostId = ent.BALANCEPOSTID,
                               BalancePostName = ent.BALANCEPOSTNAME,
                               employeeName = "",
                               setpostid = b.EMPLOYEEPOSTID
                           }).ToList();
            }
            else
            {
            ...
            EmployeeBLL bllEmployee = new EmployeeBLL();
            List<T_HR_EMPLOYEE> entEmployees = new List<T_HR_EMPLOYEE>();
            foreach (var item in PostAll)
            {
                var emp = bllEmployee.GetEmployeeByPostID(item.setpostid);
                entEmployees.AddRange(emp.ToList());
                foreach (var empitem in emp)
                {
                    Tracer.Debug("员工已设置考勤结算岗位：" + empitem.EMPLOYEECNAME + " 指定的结算岗位为：" + item.BalancePostName);
                }
            }
            return entEmployees;
        }
    ...
    public class EpmloyeeBlanceSet
    {
