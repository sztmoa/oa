...
namespace SMT
{
    ...
    public sealed class MSOracle
    {
        ...
        public static string ConnectionString
        {
            get
            {
                object strConn = SMT.SMTCache.GetCache("OracleConnectString");
                if (strConn == null)
                {
                    strConn = System.Configuration.ConfigurationManager.AppSettings["OracleConnectString"].ToString();
                    SMT.SMTCache.Insert("OracleConnectString", conn);
                }
                //string strConn = ConfigurationManager.AppSettings["conString"];
        ...
        public static string GetOneValue(string sqlString)
        {
            using (OracleConnection connection = new OracleConnection(ConnectionString))
            {
                ...
                {
                    connection.Open();
                    object obj = ExecuteScalar(connection, CommandType.Text, sqlString);
       ...
       /// <summary>
        ///  通过事务执行多个实体操作 ArrayList: OracleParameter[]
       /// </summary>
       /// <param name="sql"></param>
        /// <param name="arls">ArrayList: OracleParameter[]</param>
        public static bool ExecuteSQLTransactionByModel(string sql,ArrayList arls)
        {
            bool bnl = false;
            using (OracleConnection connection = new OracleConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    {
                        OracleParameter[] param = (OracleParameter[])arls[i];
                    ...
                    //foreach (System.Collections.DictionaryEntry item in has)
                    //{
                    //    string sql = item.Key.ToString();
                    //    OracleParameter[] param = (OracleParameter[])item.Value;
                    //    OracleCommand command = connection.CreateCommand();
                    //    command.Transaction = transaction;
                    //    command.CommandText = sql;
                    //    foreach(OracleParameter op in param)
                    ...
                    //    // swapStr = swapStr.Replace(item.Key.ToString(), item.Value.ToString());
                    //}
                    transaction.Commit();
                    bnl = true;
                }
        ...
        /// <returns></returns>
        public static DataTable GetDataTableByStoredProcedure(string spName, params object[] parameterValues)
        {
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                OracleParameter[] commandParameters = MSOracleProviderParameterCache.GetSpParameterSet(ConnectionString, spName);
        ...
        /// </summary>
        /// <param name="spname">存储过程名称</param>
        /// <param name="commandParameters">参数params OracleParameter[]</param>
        /// <returns></returns>
        public static DataTable ExecuteDataTable(string spname, params OracleParameter[] commandParameters)
        {
            //create a command and prepare it for execution
            Open();
            OracleCommand cmd = new OracleCommand();
    ...
    public sealed class MSOracleProviderParameterCache
    {
        ...
        //instances from being created with "new MSOracleProviderParameterCache()".
