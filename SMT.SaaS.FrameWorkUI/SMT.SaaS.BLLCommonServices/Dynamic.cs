...
#pragma warning restore 0219
        ...
        Expression ParseComparison()
        {
            ...
            while (token.id == TokenId.Equal || token.id == TokenId.DoubleEqual ||
                ...
                token.id == TokenId.LessThan || token.id == TokenId.LessThanEqual || token.id == TokenId.Like)
            {
                Token op = token;
                NextToken();
        ...
        Expression GenerateLike(Expression left, Expression right)
        {
            ...
            //{
            //    MethodInfo method = left.Type.GetMethod("EndsWith", new[] { typeof(string) });
            //    return Expression.Call(left, method, new[] { right });
            //}
            //else if (value.EndsWith("%"))
            //{
            //    MethodInfo method = left.Type.GetMethod("StartsWith", new[] { typeof(string) });
            //    return Expression.Call(left, method, new[] { right });
            //}
        ...
        {
            return Expression.Constant(value.Replace("%", String.Empty), typeof(string));
        }
        #endregion
